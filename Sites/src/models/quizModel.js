...
function SelectQuiz(idUsuario, pontuacao) {
    ...
    var instrucao = `SELECT pontuacao FROM quiz  WHERE fkUsuario = '${idUsuario}' order by horario`;
    console.log("Executando a instrução SQL: \n" + instrucao);
...
function buscarQuiz(idUsuario) {
    ...
    var instrucaoSql = `SELECT pontuacao FROM quiz  WHERE fkUsuario = '${idUsuario}' order by horario`;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
